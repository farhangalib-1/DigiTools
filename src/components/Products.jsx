    ...
    <div>
        <div className="TextContents w-10/12 mx-auto mt-30 text-center">
            ...
            <button onClick={()=>{setActive("Products")}} className={`text-[16px] 
                ${active==='Products'? 'bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white':"text-black bg-transparent" } px-5 py-2 rounded-full font-semibold  shadow-2xl cursor-pointer`}>Products</button>
            <button onClick={()=>{setActive("Cart")}} className={`text-[16px]  ${active==='Cart'? 'bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white cursor-pointer':"text-black bg-transparent" } px-5 py-2 rounded-full font-semibold cursor-pointer`}>Cart</button>
            </div>
        </div>
    </div>
  )
